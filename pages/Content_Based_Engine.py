...
# Sayfa Ayarları
st.set_page_config(
    page_title="Content Based Product Recommendation Systems",
...
st.title("Content Based Product Recommendation System")
st.image("images/content.png")

content_df = pd.read_csv("content_df.csv")


def content_recommend_products(df, product_id_encoded):
...
product_id_encoded= st.sidebar.number_input("Number of Product_id", min_value=1, format="%d")


df_products=content_recommend_products(content_df,product_id_encoded,)
...
if st.sidebar.button("Submit"):
    ...
    st.table(content_recommend_products(content_df,product_id_encoded))
